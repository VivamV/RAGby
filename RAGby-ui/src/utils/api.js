...
api.interceptors.response.use(
 ...
 (error) => {
   if (error.response?.status === 401) {
     ...
     if (!isAuthRequest) { //unauthorised request ke liye redirect karne ko
       // Only clear auth data for non-auth requests (e.g., expired tokens)
...
export const authAPI = {
 ...
 login: async (email, password) => {
   const fixedSalt = '$2b$12$ABCDEFGHIJKLMNOPQRSTUu'; // Fixed salt for deterministic hashing
  ...
  signup: async (email, password, name) => {
   // Use the same deterministic hashing with fixed salt
   const fixedSalt = '$2b$12$ABCDEFGHIJKLMNOPQRSTUu'; // Same fixed salt
   const hashedPassword = await bcrypt.hash(password, fixedSalt);
